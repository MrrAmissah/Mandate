...
- `003_idempotency_http_metadata.up.sql` synchronizes committed idempotency status and stable response-header metadata and rejects unknown mutation scopes.
- `003_idempotency_http_metadata.down.sql` removes only that trigger/function and its migration registry row.
