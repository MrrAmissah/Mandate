...
import { createActionAttemptExpiryHealthServer } from './action-attempt-expiry-health.js';
...
function safeHost(value, fallback) {
  ...
  if (typeof host !== 'string' || host.length < 1 || host.length > 255 || /\s/.test(host)) {
    throw new Error('MANDATE_EXPIRY_HEALTH_HOST must be a non-empty hostname or address without whitespace.');
  }
  return host;
}
...
export function parseActionAttemptExpiryConfig(env = process.env) {
  ...
  const pollIntervalMs = integer(env.MANDATE_EXPIRY_POLL_INTERVAL_MS, 1000, {
    name: 'MANDATE_EXPIRY_POLL_INTERVAL_MS', minimum: 100, maximum: 60000
  });
  const readinessStaleMs = integer(
    env.MANDATE_EXPIRY_READINESS_STALE_MS,
    Math.max(5000, pollIntervalMs * 3),
    {
      name: 'MANDATE_EXPIRY_READINESS_STALE_MS',
  ...
  return Object.freeze({
    ...
    readinessStaleMs,
    readinessFailureThreshold: integer(env.MANDATE_EXPIRY_READINESS_FAILURE_THRESHOLD, 3, {
      name: 'MANDATE_EXPIRY_READINESS_FAILURE_THRESHOLD', minimum: 1, maximum: 100
    }),
    healthHost: safeHost(env.MANDATE_EXPIRY_HEALTH_HOST, '127.0.0.1'),
    healthPort: integer(env.MANDATE_EXPIRY_HEALTH_PORT, 8788, {
      name: 'MANDATE_EXPIRY_HEALTH_PORT', minimum: 1, maximum: 65535
    })
  });
}
...
export class ActionAttemptExpiryProcess {
  constructor({
    ...
    if (!worker?.drain || !worker?.backlog) throw new TypeError('An observable expiry worker is required.');
  ...
  readiness(now = this.clock()) {
    ...
    if (!this.running || !this.metrics.lastSuccessAt) {
      return Object.freeze({ ready: false, reason: 'STARTING', ...common });
    }
  ...
  async run({ signal } = {}) {
    if (this.running) throw new Error('The action-attempt expiry process is already running.');
    ...
    try {
      while (!signal?.aborted) {
        try {
          ...
          // Safe structured failure state is retained while the bounded loop continues.
        }
...
export async function createActionAttemptExpiryRuntime({ env = process.env, logger = console } = {}) {
  ...
  try {
    ...
    const expiryProcess = new ActionAttemptExpiryProcess({
      worker,
