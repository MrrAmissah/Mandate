...
test('expiry executable handles signals, closes runtime, and reports safe startup failure', async () => {
...
test('expiry runtime checks schema readiness without applying migrations or using API keys', async () => {
  ...
  assert.doesNotMatch(source, /applyMigrations|MANDATE_API_KEY|createRuntime/);
