...
test('approval expiry migration makes deadline state durable and database-authoritative', async () => {
  ...
  assert.match(sql, /approvals_expiring_scope_idx/);
  ...
  assert.match(sql, /approval consumption cannot commit after expiry deadline/);
  assert.match(sql, /approval is unavailable for a new active assignment/);
  assert.match(sql, /NEW\.status = 'EXPIRED' AND OLD\.status IN \('PENDING', 'APPROVED'\)/);
  ...
  assert.match(sql, /event\.data ->> 'previousStatus' = OLD\.status/);
...
test('approval expiry development rollback removes only expiry-owned schema changes', async () => {
  ...
  assert.match(sql, /^BEGIN;/);
  assert.match(sql, /COMMIT;\s*$/);
  assert.match(sql, /DROP INDEX IF EXISTS mandate\.approvals_expiring_scope_idx/);
