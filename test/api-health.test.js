...
test('readiness pool is isolated and acquisition-bounded', async () => {
  ...
  await assert.rejects(
    createPostgresHealthPool({
      connectionString: 'postgresql://unused:unused@127.0.0.1:1/unused',
      connectionTimeoutMillis: 99
    }),
    /between 100 and 10000/
  );
});
...
test('PostgreSQL readiness checks the required migration with a bounded query timeout', async () => {
  ...
  assert.deepEqual(await health.readiness(), {
    ...
    reason: 'READY',
    mode: 'postgres',
    requiredMigration: '011_approval_assignments',
    databaseObservedAt: '2026-07-30T00:00:01.000Z',
    checkedAt: '2026-07-30T00:00:00.000Z'
  });
  assert.equal(calls.length, 1);
  assert.equal(calls[0].query_timeout, 750);
  assert.deepEqual(calls[0].values, ['011_approval_assignments']);
});
